...
class InstructorRemoteDataSourceImpl implements InstructorRemoteDataSource {
  ...
  Future<InstructorEntity?> getInstructor(String visitorId) async {
    ...
    return InstructorEntity(
      ...
      name: teacher?['display_name'] ?? profile['name'] ?? profile['email']?.split('@').first ?? 'Instructor',
      avatarUrl: teacher?['avatar_url'] ?? profile['avatar_url'],
      coverImageUrl: null,
      headline: _sanitizeText(profile['headline_ar']) ?? _sanitizeText(profile['headline_en']),
      bio: _sanitizeText(profile['bio_ar']) ?? _sanitizeText(profile['bio_en']) ?? teacher?['bio'],
      expertise: profile['expertise'] != null ? List<String>.from(profile['expertise'] as List) : null,
