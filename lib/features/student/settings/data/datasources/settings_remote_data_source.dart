...
import 'package:lms_platform/features/student/settings/data/models/settings_model.dart';
import 'package:lms_platform/features/student/settings/data/models/user_profile_model.dart';
...
class SettingsRemoteDataSourceImpl implements SettingsRemoteDataSource {
  ...
  Future<UserProfileModel> getUserProfile(String userId) async {
    ...
    final profileData = {
      ...
      if (response['role'] == 'instructor') ...{
        ...
        'cover_image_url':
            teacherData?['cover_image_url'] ?? response['avatar_url'],
      },
    };
  ...
  Future<UserProfileModel> updateUserProfile(
    ...
    if (isInstructor && teacherFields.isNotEmpty) {
      ...
      if (!teacherFields.containsKey('display_name') &&
          profileFields.containsKey('name')) {
        teacherFields['display_name'] = profileFields['name'];
      }
      // Sync teacher avatar if profiles.avatar_url is updated
      if (!teacherFields.containsKey('avatar_url') &&
          profileFields.containsKey('avatar_url')) {
      ...
      await client.from('teachers').upsert({
        'profile_id': userId,
        'display_name':
            teacherFields['display_name'] ?? profileMeta?['name'] ?? 'مدرس',
        ...teacherFields,
