...
import 'package:lms_platform/features/instructor_dashboard/presentation/widgets/students_progress_sheet_models.dart';
import 'package:lms_platform/features/instructor_dashboard/presentation/widgets/students_progress_sheet_widgets.dart';
import 'package:lms_platform/features/instructor_dashboard/presentation/widgets/students_progress_sheet_table.dart';
import 'package:lms_platform/features/instructor_dashboard/presentation/widgets/students_progress_sheet_attempts.dart';
import 'package:lms_platform/features/instructor_dashboard/presentation/widgets/students_progress_sheet_lessons.dart';
...
class _StudentsProgressSheetScreenState
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final rows = builders.values
          .map((b) => b.build(quizByStudent[b.uid] ?? []))
          .toList();
      ...
      _filtered.sort((a, b) {
        ...
        switch (column) {
          case 'name':
            ...
            break;
          case 'courses':
            cmp = a.coursesCount.compareTo(b.coursesCount);
            break;
          case 'progress':
            cmp = a.avgProgress.compareTo(b.avgProgress);
            break;
          case 'lessons':
            cmp = a.completedLessons.compareTo(b.completedLessons);
            break;
          case 'watch':
            cmp = a.watchSeconds.compareTo(b.watchSeconds);
            break;
          case 'quizzes':
            cmp = a.quizCount.compareTo(b.quizCount);
            break;
          case 'score':
            cmp = a.avgQuizScore.compareTo(b.avgQuizScore);
            break;
          case 'last':
            cmp = (a.lastActivity ?? DateTime(2000))
                .compareTo(b.lastActivity ?? DateTime(2000));
            break;
          default:
            cmp = 0;
        }
  ...
  void _exportCsv(bool isArabic) {
    ...
    for (int i = 0; i < _filtered.length; i++) {
      final r = _filtered[i];
      final watchStr =
          '${r.watchSeconds ~/ 3600}h ${(r.watchSeconds % 3600) ~/ 60}m';
      buffer.writeln(
    ...
    _saveAndShare(
        buffer.toString(),
        'students_progress_${DateFormat('yyyyMMdd_HHmm').format(DateTime.now())}.csv',
        isArabic);
  }

  Future<void> _saveAndShare(
      String content, String fileName, bool isArabic) async {
    try {
      if (kIsWeb) {
        downloadFile(content, fileName);
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content:
              Text(isArabic ? 'تم التصدير بنجاح' : 'Exported successfully'),
        ));
      } else {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Column(
          ...
          children: [
            Text(isArabic ? 'جدول تقدم الطلاب' : 'Students Progress Sheet',
                style:
        ...
        actions: [
          if (!_isLoading && _filtered.isNotEmpty)
            IconButton(
                onPressed: () => _exportCsv(isArabic),
                icon: const Icon(Icons.table_chart_outlined),
                tooltip: isArabic ? 'تصدير CSV' : 'Export CSV'),
          IconButton(
  ...
  Widget _buildSearchAndStats(bool isDark, bool isArabic) {
    final avgProgress = _rows.isEmpty
        ? 0.0
        : _rows.map((r) => r.avgProgress).reduce((a, b) => a + b) /
            _rows.length;
        ...
        Container(
          ...
          child: Row(children: [
            ...
            SheetStatChip(
                icon: Icons.quiz,
                value: '$totalQuizzes',
                label: isArabic ? 'محاولة اختبار' : 'Quiz Attempts',
